...
module Jekyll
  class ExifTag < Liquid::Block
    ...
    def render(context)
      ...
      Jekyll.logger.debug 'ExifTag: tag', tag
      ...
      # Jekyll.logger.debug 'ExifTag: sources', sources
      # Jekyll.logger.debug 'ExifTag: img', img
      ...
      # then start testing with the sources from _config.yml
        ...
        Jekyll.logger.debug 'ExifTag: file_name', file_name
      end
      # try it and return empty string on failure
      begin
        exif = EXIFR::JPEG.new(file_name)
        Jekyll.logger.debug 'ExifTag: exif', exif
